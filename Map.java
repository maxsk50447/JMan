...
public class Map implements ActionListener{
    ...
    public void putNew(int typ, int x, int y){
        if(isEmpty(x, y)){
        	if(typ == Piece.BLOCK) grid[x][y] = new Block(x, y, this);
        	else if(typ == Piece.JMAN){
        		jMan = new JMan(x, y, Piece.rand(0, 2), this);
        		grid[x][y] = jMan;
        	} 
        	else if(typ == Piece.WALKER) grid[x][y] = new Walker(x, y, Piece.rand(0, 2), this);
        	else if(typ == Piece.PILLAR) grid[x][y] = new Pillar(x, y, Piece.rand(0, 2), this);
